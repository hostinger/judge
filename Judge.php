...
class Judge
{
    ...
    public function isValid($word)
    {
        ...
        foreach ($this->database['reserved_app_names']['regexp'] as $record) {
            if (preg_match("/$record/i", $word)) throw new Exception("This word " . $word . " is forbidden");
        }
        // check by phishing words
        foreach ($this->database['reserved_app_names']['phishing_words'] as $record) {
            if (stripos($word, $record) !== false) throw new Exception("This word " . $word . " is forbidden");
        }
        // check by brands
